...
function handleDisabledNavClick(event) {
...
function bindDisabledNavigation() {
  const disabledLinks = document.querySelectorAll('nav a.disabled-link');
  if (!disabledLinks.length) return;

  disabledLinks.forEach((link) => {
    link.removeEventListener('click', handleDisabledNavClick);
    link.addEventListener('click', handleDisabledNavClick);
  });
}
...
function bindMenuAutoClose() {
  const navLinks = document.querySelectorAll('nav a[href]:not(.disabled-link)');
